...
- `source_markdown`: 비교할 Markdown 경로. 비워 두면 HWP/HWPX는 `.github/scripts/resolve_extracted_name.py`의 정본 파일명 규칙으로 자동 추출본을 탐색
...
- HWP 5.x는 Kordoc 다페이지 HTML 렌더 → headless Edge/Chrome PDF → OneOCR 경로를 사용합니다.
- HWPX는 Kordoc 전체 페이지 SVG 스택 → headless Edge/Chrome PDF → OneOCR 경로를 유지합니다.
- HWP3는 기본 Markdown 자동추출은 되지만 Kordoc 시각 렌더 대상이 아니므로 OneOCR 입력에서 조기에 거부됩니다.
- 결과는 정본 자동추출 파일명에 맞춘 `extracted/<추출명>.oneocr.md` sidecar로 남습니다. 같은 stem의 HWP/HWPX가 함께 있으면 `문서-hwp.oneocr.md`, `문서-hwpx.oneocr.md`처럼 suffix를 유지하고 이전 대체 sidecar는 정리합니다.

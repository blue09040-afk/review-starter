...
중요한 HWPX 또는 PDF를 자동추출 결과와 한 번 더 비교하고 싶을 때만 `OneOCR Cross Check`를 사용합니다. (HWP 5.x는 후속 workflow 이식 후 지원 예정)

GitHub의 **Actions → OneOCR Cross Check → Run workflow**에서 다음을 입력합니다.

- `document_path`: 저장소 기준 HWPX 또는 PDF 경로
- `source_markdown`: 비교할 Markdown 경로. 비워 두면 HWPX는 현재 starter workflow 기준 `extracted/<원본 stem>.md`만 자동 탐색함. 같은 stem의 다른 형식이 함께 있어 자동추출본에 `-hwpx` 같은 suffix가 붙은 경우에는 이 값을 직접 지정
...
- 현재 starter의 `oneocr-cross-check.yml`은 **HWPX·PDF** 입력만 받습니다. HWP 5.x 시각 교차검증(HTML 렌더 → PDF 인쇄 → OneOCR)은 원본 `review`에 있으며 starter 후속 이식 대상입니다.
- HWP3는 기본 Markdown 자동추출은 되지만 OneOCR 입력 대상이 아닙니다.
- 결과는 현재 starter workflow 기준 `extracted/<원본 stem>.oneocr.md` sidecar로 남습니다. 같은 stem의 HWPX·PDF를 각각 교차검증하면 같은 sidecar 경로를 사용할 수 있으므로 주의해야 하며, 충돌 회피용 정본 파일명 연동은 HWP 5.x workflow와 함께 후속 이식 대상입니다.
- Kordoc의 HWPX 시각 렌더는 머리말·꼬리말·수식을 완전 렌더하지 않을 수 있으므로, 해당 요소가 중요하면 OneOCR만으로 원문 충실성을 확정하지 마세요.
